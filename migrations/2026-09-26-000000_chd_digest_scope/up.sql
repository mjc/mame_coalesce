...
CREATE TABLE asset_requirements_chd_scope (
...
SELECT
    snapshot_key, set_name, component_order, asset_name, role, size, crc, md5, sha1,
    CASE WHEN role = 'disk' AND evidence_scope = 'disk_data'
         THEN 'chd_header_sha1' ELSE evidence_scope END,
    evidence_provenance, merge_name, dump_status, serial, date,
    metadata_json, source_line, source_column
FROM asset_requirements;

DROP TABLE asset_requirements;
ALTER TABLE asset_requirements_chd_scope RENAME TO asset_requirements;
...
ADD COLUMN evidence_scope TEXT NOT NULL DEFAULT 'whole_asset'
CHECK (evidence_scope IN ('whole_asset', 'chd_header_sha1', 'unknown'));
UPDATE software_components
SET evidence_scope = 'chd_header_sha1'
WHERE component_kind = 'disk';
