...
CREATE TABLE asset_requirements_previous_scope (
...
INSERT INTO asset_requirements_previous_scope (
...
SELECT
    snapshot_key, set_name, component_order, asset_name, role, size, crc, md5, sha1,
    CASE WHEN evidence_scope = 'chd_header_sha1'
         THEN 'disk_data' ELSE evidence_scope END,
    evidence_provenance, merge_name, dump_status, serial, date,
    metadata_json, source_line, source_column
FROM asset_requirements;

DROP TABLE asset_requirements;
ALTER TABLE asset_requirements_previous_scope RENAME TO asset_requirements;
...
ALTER TABLE software_components DROP COLUMN evidence_scope;
